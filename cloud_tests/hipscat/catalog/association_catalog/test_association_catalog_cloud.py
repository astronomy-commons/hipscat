...
def test_empty_directory(tmp_dir_cloud, example_cloud_storage_options, association_catalog_info_data, association_catalog_join_pixels):
    ...
    with pytest.raises(FileNotFoundError):
        AssociationCatalog.read_from_hipscat(empty_path, storage_options=example_cloud_storage_options)

    catalog_path = os.path.join(tmp_dir_cloud, "empty")
    file_io.make_directory(catalog_path, storage_options=example_cloud_storage_options, exist_ok=True)
