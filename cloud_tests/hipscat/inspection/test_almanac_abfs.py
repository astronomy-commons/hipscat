...
def test_catalogs_filters(default_almanac_abfs):
    """Test listing names of catalogs, using filters"""
    ## all (non-deprecated) catalogs
    assert len(default_almanac_abfs.catalogs()) == 8

    ## **all** catalogs
    assert len(default_almanac_abfs.catalogs(include_deprecated=True)) == 9

    ## all object and source (skip association/index/etc)
    assert len(default_almanac_abfs.catalogs(include_deprecated=True, types=["object", "source"])) == 6

    ## all active object and source
    assert len(default_almanac_abfs.catalogs(types=["object", "source"])) == 5

    ## non-existent type
    assert len(default_almanac_abfs.catalogs(types=["foo"])) == 0


def test_linked_catalogs_object(default_almanac_abfs):
    """Check that we can access the affiliated catalogs"""
    object_almanac = default_almanac_abfs.get_almanac_info("small_sky")
    ...
    source_almanac = default_almanac_abfs.get_almanac_info(object_almanac.sources[0].catalog_name)
    assert source_almanac.catalog_name == "small_sky_source_catalog"

    source_catalog = default_almanac_abfs.get_catalog(object_almanac.sources[0].catalog_name)
...
def test_linked_catalogs_association(default_almanac_abfs):
    """Check that read almanac entries are fully linked to one another."""

    association_almanac = default_almanac_abfs.get_almanac_info("small_sky_to_small_sky_order1")
...
def test_linked_catalogs_index(default_almanac_abfs):
    """Check that read almanac entries are fully linked to one another."""

    index_almanac = default_almanac_abfs.get_almanac_info("index_catalog")
...
def test_linked_catalogs_margin(default_almanac_abfs):
    """Check that read almanac entries are fully linked to one another."""

    margin_almanac = default_almanac_abfs.get_almanac_info("margin_cache")
...
def test_get_catalog(default_almanac_abfs):
    """Test that catalogs in almanac really exist (in test directory)"""

    for catalog_name in default_almanac_abfs.catalogs():
        catalog = default_almanac_abfs.get_catalog(catalog_name)
        assert catalog
