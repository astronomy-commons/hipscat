...
def test_write_string_to_file(tmp_dir_abfs, example_abfs_storage_options):
    test_file_path = os.path.join(tmp_dir_abfs, "text_file.txt")
...
def test_load_json(small_sky_dir_local, small_sky_dir_abfs, example_abfs_storage_options):
    catalog_cloud_path = os.path.join(small_sky_dir_abfs, "catalog_info.json")
    ...
    json_dict_cloud = load_json_file(catalog_cloud_path, storage_options=example_abfs_storage_options)
    json_dict_local = load_json_file(catalog_info_pointer, encoding="utf-8")
    assert json_dict_cloud == json_dict_local


def test_load_csv_to_pandas(small_sky_dir_local, small_sky_dir_abfs, example_abfs_storage_options):
    partition_info_path = os.path.join(small_sky_dir_local, "partition_info.csv")
    partition_info_path_cloud = os.path.join(small_sky_dir_abfs, "partition_info.csv")
...
def test_load_parquet_to_pandas(small_sky_dir_local, small_sky_dir_abfs, example_abfs_storage_options):
    pixel_data_path = pixel_catalog_file(small_sky_dir_local, 0, 11)
    pixel_data_path_cloud = pixel_catalog_file(small_sky_dir_abfs, 0, 11)
...
def test_write_df_to_csv(tmp_dir_abfs, example_abfs_storage_options):
    random_df = pd.DataFrame(np.random.randint(0, 100, size=(100, 4)), columns=list("ABCD"))
    test_file_path = os.path.join(tmp_dir_abfs, "test.csv")
    test_file_pointer = get_file_pointer_from_path(test_file_path)
...
def test_read_parquet_data(tmp_dir_abfs, example_abfs_storage_options):
    random_df = pd.DataFrame(np.random.randint(0, 100, size=(100, 4)), columns=list("ABCD"))
    test_file_path = os.path.join(tmp_dir_abfs, "test.parquet")
